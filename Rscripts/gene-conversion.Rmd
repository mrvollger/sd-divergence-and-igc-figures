...
conflict_prefer("rename_with", "dplyr")
conflict_prefer("select", "dplyr")
conflict_prefer("summarise", "dplyr")
conflict_prefer("group_by_at", "dplyr")
...
out_igc <- IGC %>%
    ...
    select(-thickStart, -thickEnd, -color, -strand, -score, -status, -cum.bp, -Sex, -Population) %>%
...
merged_df$pvalue <- get_pvalues_from_igc(merged_df)
out_merged_df <- merged_df %>%
    ...
    select(-thickStart, -thickEnd, -igc_count, -`cum.bp`, -pvaluemerged_df) %>%
    as.data.table()
...
sum_df <- bind_rows(
    ...
    mutate(
        Superpopulation = case_when(
            !startsWith(Category, "By sample") ~ "NA",
            TRUE ~ as.character(Superpopulation)
        ),
...
igc_genes_of_interest <- merged_df %>%
    filter(pvalue <= 0.05) %>%
    filter(
        grepl(igc_gene_pat, donor_gene_names) |
            grepl(igc_gene_pat, acceptor_gene_names)
    ) %>%
    mutate(
        gene_of_interest = str_extract(acceptor_gene_names, igc_gene_pat)
    ) %>%
...
z <- merged_df[(pvalue < 0.05) & (reference_start > 19e6) & (reference_end < 30e6) & (reference_name == "chr15"), ]
z$gene_of_interest <- "chr15_19_30"
y <- merged_df[(pvalue < 0.05) & (reference_start > 21260474) & (reference_end < 22866350) & (reference_name == "chr16"), ]
y$gene_of_interest <- "chr16_NPIP_1"
w <- merged_df[(pvalue < 0.05) & (reference_start > 21323835) & (reference_end < 21747698) & (reference_name == "chr16"), ]
w$gene_of_interest <- "chr16_NPIP_2"

igc_genes_of_interest <- rbind(igc_genes_of_interest, z, y, w)
...
regions_of_interest <- regions_of_interest[
    gene_of_interest %in% c("chr15_19_30", "chr16_NPIP_1", "chr16_NPIP_2")
]
i <- 1
...
for (i in 1:nrow(regions_of_interest)) {
    ...
    cur_igc <-
        ...
        filter(
            gene_of_interest == my_gene_of_interest,
            name >= min_snv,
            n >= min_sample,
        ) %>%
    ...
    df_genes <- gene_bed6 %>%
        ...
        ungroup() %>%
        mutate(
            y = disjointBins(IRanges(start, end) + my_len / 50),
    ...
    fig_height <- (max(11, nrow_cur_gc / 6 + 1)) * scale
    gene_height <- length(unique(df_genes$y)) / 3
    fig_patch <- p_gc + p_hist + p_genes + plot_spacer() + plot_cov +
        plot_layout(
            widths = c(5, 1),
            heights = c(fig_height - 2, gene_height, 1),
            nrow = 3, ncol = 2
        ) & theme(
        plot.background = element_rect(fill = "transparent", color = "transparent")
    )
    ggsave(glue("{odir}/genes/2022-10-03/{gene_of_interest}_{chrm}_min_snv_{min_snv}_gc_fig.pdf"),
        limitsize = FALSE,
        # height=12,
        height = fig_height,
        width = 16 * scale,
...
system(glue("pdfunite figures/genes/2022-10-03/*min_snv_{min_snv}*.pdf figures/2022-10-03/igc_all_genes_booklet_min_snv_{min_snv}.pdf"))
print("hi")
```
