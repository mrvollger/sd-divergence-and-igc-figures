...
for( population in c("", "_population") ) { #
    for( SD_var in c("SD", "SDnoIGC", "SDIGC") ){
        ...
        spectra <- fread(glue("data/mutyper-results{IS_IGC}/tables/stratify/spectra_{SD_var}_Unique{population}.tbl")) %>%
            ...
            as.data.table()
        spectra
        print( spectra %>% group_by(stratify) %>% summarise(sum(count)/1e6,sum(target_count)/1e6, sum(count)/sum(target_count) ) )
        ...
        mutable_triplets.df = spectra %>% 
            ...
            arrange(-mutable) %>%
            pivot_wider(
                names_from = c("stratify"), values_from = c("mutable", "triplet_frac"),
                id_cols = c("first_three_bases"),
            ) %>%
            mutate(
                x = mutable_Unique,
        ...
        mutable_triplets = mutable_triplets.df %>%
            #filter(!CpG) %>%
            ggplot(aes(x=x,y=y, label=first_three_bases)) +
            geom_text_repel(min.segment.length=0) +
            geom_smooth(method="lm", se=F, formula = y~x) +
            geom_point() +
            stat_cor(method = "pearson") + 
            #stat_regline_equation(label.y = 30) + 
            scale_x_continuous("Frequency of a mutation in unique sequence for all triplet contexts", trans="log10") +
            annotation_logticks(side="b")+
            ylab(TeX("$ \\; \\frac{SD \\; triplet \\; composition}{Unique \\; triplet \\; composition}$")) +
