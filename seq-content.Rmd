...
for (population in c("")) { # , "_population"
    for (SD_var in c("SD", "SDnoIGC", "SDIGC")) { #
        ...
        mutable_triplets <- mutable_triplets.df %>%
            ...
            scale_x_continuous("Frequency of mutation in unique sequence", trans = "log10") +
            annotation_logticks(side = "b") +
            ylab(TeX("$ \\; \\frac{SD \\; composition}{Unique  \\; composition}$")) +
            theme_minimal_grid() # ggsave("tmp.pdf")
        mutable_triplets
        mutable_triplets.df
        ########################################################################################
        ########################################################################################
...
SD_var <- "SD"
f1 <- glue("data/mutyper-results{IS_IGC}/spectra/stratify/Unique_targets.txt")
f2 <- glue("data/mutyper-results{IS_IGC}/spectra/stratify/{SD_var}_targets.txt")
f3 <- glue("data/mutyper-results{IS_IGC}/spectra/stratify/Unique_spectra.txt")
f4 <- glue("data/mutyper-results{IS_IGC}/spectra/stratify/{SD_var}_spectra.txt")

f1 <- "https://eichlerlab.gs.washington.edu/help/mvollger/share/ksfs/Unique_targets.txt"
f2 <- "https://eichlerlab.gs.washington.edu/help/mvollger/share/ksfs/SD_targets.txt"
f3 <- "https://eichlerlab.gs.washington.edu/help/mvollger/share/ksfs/Unique_spectra.txt"
f4 <- "https://eichlerlab.gs.washington.edu/help/mvollger/share/ksfs/SD_spectra.txt"

unique_targets <- fread(f1)
sd_targets <- fread(f2)
targets <- bind_rows(
    list(SD = sd_targets, Unique = unique_targets),
    .id = "stratify"
...
unique_spectra <- fread(f3)
sd_spectra <- fread(f4)
spectra <- bind_rows(
    list(SD = sd_spectra, Unique = unique_spectra),
    .id = "stratify"
)

spectra_long <- spectra %>%
    pivot_longer(!c("stratify", "sample"), names_to = "spectra", values_to = "count") %>%
    mutate(
        first_three_bases = substr(spectra, 1, 3)
    ) %>%
    merge(targets, by = c("first_three_bases", "stratify")) %>%
    group_by(stratify) %>%
    mutate(
        target_percent = count / target_count,
        target_percent_norm = target_percent / sum(target_percent)
    ) %>%
    data.table()
sum(spectra_long$target_percent_norm) == 2


spectra_wide <- spectra_long %>%
    pivot_wider(
        id_cols = c("stratify", "sample"),
        names_from = c("spectra"),
        values_from = c("target_percent_norm")
    ) %>%
    data.table()

spectra_mat <- clr(spectra_wide[, -c("stratify", "sample")])

spectra_pca <- prcomp(spectra_mat, scale. = TRUE)
# sort(spectra_pca$rotation[, 1])

autoplot(spectra_pca,
    data = spectra_wide,
    shape = "stratify",
) +
    theme(
