...
pdf(glue("{odir}/intra-gc-with-freq-at-least-{min_n}.pdf"),
    height = 7, width = 14
)
kp <- plotKaryotype(
...
make_gc_ideogram(a, d, NOYM, "figures/all_border_gc_events.pdf")
...
IGC %>%
    ...
    rename(setNames(names(.), gsub("\\.liftover", ".acceptor", names(.)))) %>%
    rename(setNames(names(.), gsub(".*reference_name", "chrm", names(.)))) %>%
    rename(setNames(names(.), gsub("reference_", "", names(.)))) %>%
...
pli_plot_c <- (pli_plot.df %>%
    ...
    add_repel_pli(pli = 0.75, min_donor = 50, y = "N_IGC_Donor")
...
ggsave("figures/igc_pli.pdf", plot = pli_plot, height = 8, width = 12)
``` 
...
igc_cov <- fread("data/windowed_acceptor_donor_counts.bed") %>%
    ...
    mutate(per = ecdf(value)(value)) %>%
    data.table()
igc_cov


xx <- igc_cov %>%
    mutate(value = case_when(value > n_samples ~ n_samples, TRUE ~ value)) %>%
    ggplot(aes(fill = name, x = value)) +
    geom_bar(position = "dodge", alpha = 0.8) +
    # facet_col(~name) +
    ylab("Count of genomic windows (1 kbp)") +
    xlab("Number of donors or acceptors") +
    geom_text_repel(
        data = igc_cov %>% sample_n(1),
        aes(x = n_samples, y = 0, label = glue("{n_samples}+")),
        nudge_y = 1e3
    ) +
...
ggsave("figures/igc_coverage_hist.pdf", plot = xx)

quantile(igc_cov$value, probs = seq(0, 1, 0.25))
igc_cov %>%
    group_by(name) %>%
    summarise(x = list(quantile(value, probs = c(0.95, .997)))) %>%
    data.table()
```
...
igc_recur_df <- merged_df %>%
    ...
    mutate(
        ...
        st1 = round(reference_start.liftover - flank),
        en1 = round(reference_start.liftover),
        st2 = round(reference_end.liftover),
        en2 = round(reference_end.liftover + flank),
        ID = paste(chrm, en1, st2, "IGC", sep = "-"),
        ave_support = name,
    ) %>%
    filter(!(gene_of_interest == "F8" & chrm != "chrX")) %>%
    select(chrm, st1, en1, st2, en2, ID, gene_of_interest, igc_count, ave_support) %>%
